        ...
        for (int k2 = 0;k2 < 4;k2++) {
            for (int k3 = 0;k3 < 4;k3++) {
                for (int k4 = 1;k4 <= 5;k4++) {
                    ...
                    knapsack_problem ks = read_data(problems_full_path);
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Please the select an algorithm by number 1-6\n";
    std::cout << "0 = all algorithms\n";
    std::cout << "other numbers for exit\n";

    int selection = 0;
    std::cin >> selection;

    if ((selection < 0) || (selection > 6)) {
        std::cout << "Terminated by the user\n";
        exit(0);
    }
